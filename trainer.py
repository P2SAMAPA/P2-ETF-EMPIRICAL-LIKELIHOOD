...
import config
import data_manager
from empirical_likelihood import el_score
...
def main():
    ...
    for universe_name, tickers in config.UNIVERSES.items():
        print(f"\n=== Universe: {universe_name} (Empirical Likelihood) ===")
        ...
        for win in config.WINDOWS:
            ...
            for etf in tickers:
                ...
                if len(x) < 5:
                    continue
                score = el_score(x, confidence=config.CONFIDENCE)
        ...
        top_etfs = [{"ticker": ticker, "el_score": float(score), "best_window": win} for ticker, (score, win) in sorted_etfs[:config.TOP_N]]

        print(f"  Top 3 ETFs by EL lower bound: {[e['ticker'] for e in top_etfs]}")
    ...
    local_path = Path(f"results/empirical_likelihood_{today}.json")
    with open(local_path, "w") as f:
        json.dump(convert_to_serializable({"run_date": today, "universes": all_results}), f, indent=2)

    import push_results
    push_results.push_daily_result(local_path)
    print("\n=== Empirical Likelihood Engine complete ===")
